...
io.on("connection", async (socket) => {
  ...
  socket.on("friend_request", async (data) => {
    ...
    const x= await FriendRequest.create({
      sender: data.from,
      recipient: data.to,
    });
    console.log("hello",x)
  ...
  socket.on("accept_request", async (data) => {
    ...
    console.log(request_doc,"pop");

    const sender = await User.findById(request_doc.sender);
    const receiver = await User.findById(request_doc.recipient);
    console.log("noob",sender,receiver)
    sender.friends.push(request_doc.recipient);
    receiver.friends.push(request_doc.sender);

    const x=await receiver.save({ new: true, validateModifiedOnly: true });
    const y=await sender.save({ new: true, validateModifiedOnly: true });

    const z = await FriendRequest.findByIdAndDelete(data.request_id);
    console.log("bye",x,y,z)
  ...
  socket.on("get_direct_conversations", async ({ user_id }, callback) => {
    ...
    console.log("daddddddddd",existing_conversations);
  ...
  socket.on("start_conversation", async (data) => {
    ...
    console.log(existing_conversations, "Existing Conversationnnnnnnnnnn");
  ...
  socket.on("text_message", async (data) => {
    ...
    console.log("kutta",to_user,from_user)
